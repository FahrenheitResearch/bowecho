//! Fail-closed on-demand and validated-local property T-matrix table sources
...
use radar_scattering::{P3OfficialTableKind, PolarAccumulatorQuantities};
...
#[cfg(not(test))]
use crate::wrf_tmatrix_legacy_pack::load_or_download_legacy_property_luts;
use crate::wrf_tmatrix_legacy_pack::{
    LEGACY_PROPERTY_TABLE_SOURCE_BYTES, LegacyPropertyTMatrixLuts,
};
...
enum PropertyTMatrixTableStorage {
    LegacySDataPack(&'static LegacyPropertyTMatrixLuts),
...
impl PropertyTMatrixTables {
    ...
    pub fn identity(&self) -> TMatrixBandPackIdentity {
        match &self.storage {
            PropertyTMatrixTableStorage::LegacySDataPack(_) => {
    ...
    pub fn exact_frequency_hz(&self) -> f64 {
        match &self.storage {
            PropertyTMatrixTableStorage::LegacySDataPack(_) => S_BAND_RESEARCH_FREQUENCY_HZ,
            PropertyTMatrixTableStorage::ExternalValidated(bundle) => {
    ...
    pub fn borrowed_bundle(&self) -> WrfTMatrixLutBundle<'_> {
        match &self.storage {
            PropertyTMatrixTableStorage::LegacySDataPack(tables) => tables.bundle(),
            PropertyTMatrixTableStorage::ExternalValidated(bundle) => bundle.borrowed_bundle(),
        }
    }

    /// Conservative resident bytes owned by this exact table source. The
    /// on-demand S pack drops its source/archive buffers after decode and uses
    /// the decoded-table bound once; external packs retain the established
    /// twice-source bound.
    #[must_use]
    pub fn retained_source_bytes(&self) -> usize {
        match &self.storage {
            PropertyTMatrixTableStorage::LegacySDataPack(_) => embedded_lut_memory_bytes(),
            PropertyTMatrixTableStorage::ExternalValidated(bundle) => {
...
/// Deterministic shared pack root below BowEcho's override-aware model cache.
/// BowEcho installs the pinned on-demand S research pack in its fixed subdir;
/// users or a separate installer provide validated-local manifest packs.
#[must_use]
...
///
/// The byte-exact legacy S research pack is accepted only at 2.8 GHz and is
/// acquired on first use. External S/C/X selection is delegated to validated
/// local manifests and fails closed for a missing, invalid, unvalidated, or
/// ambiguous pack. There is no cross-band substitution, nearest-frequency
/// lookup, or implicit source fallback.
pub fn load_property_tmatrix_tables_exact(
    source: PropertyTMatrixTableSourceKind,
    frequency_hz: f64,
) -> Result<PropertyTMatrixTables, String> {
    load_property_tmatrix_tables_exact_with_progress(source, frequency_hz, &|_| {})
}

pub fn load_property_tmatrix_tables_exact_with_progress(
    source: PropertyTMatrixTableSourceKind,
    frequency_hz: f64,
    progress: &(impl Fn(&str) + ?Sized),
) -> Result<PropertyTMatrixTables, String> {
    validate_property_tmatrix_table_source_frequency(source, frequency_hz)?;
    match source {
        PropertyTMatrixTableSourceKind::LegacyEmbeddedSResearchV1 => Ok(PropertyTMatrixTables {
            storage: PropertyTMatrixTableStorage::LegacySDataPack(legacy_luts(progress)?),
        }),
        PropertyTMatrixTableSourceKind::ExternalValidatedPack => {
            let root = property_tmatrix_pack_cache_dir();
            std::fs::create_dir_all(&root).map_err(|error| {
...
pub fn validate_property_tmatrix_table_source_frequency(
    source: PropertyTMatrixTableSourceKind,
    frequency_hz: f64,
    ...
    {
        return Err(format!(
            "the BowEcho S research data pack exists only at exactly {S_BAND_RESEARCH_FREQUENCY_HZ} Hz; requested {frequency_hz} Hz ({band}-band); no fallback is allowed"
        ));
    }
    Ok(band)
}
...
/// Load and validate the complete on-demand five-table bundle before opening
/// heavy WRF property fields. This compatibility name is retained for callers
/// and serialized settings created while the tables were embedded.
...
/// official 2-moment or 3-moment v5.4 table; ISHMAEL needs only the on-demand
...
fn embedded_raw_evaluator() -> Result<WrfTMatrixRawEvaluator<'static>, String> {
    // Acquire outside OnceLock initialization so connectivity/filesystem
    // failures remain retryable. Only a qualified decoded bundle reaches the
    // process-lifetime evaluator cache.
    let tables = legacy_luts(&|_| {})?;
    match RAW_EVALUATOR.get_or_init(|| {
        WrfTMatrixRawEvaluator::new(tables.bundle())
            .map_err(|error| format!("validate S-pack raw property evaluator: {error}"))
    }) {
...
fn embedded_p3_raw_evaluator(
    ...
    let tables = legacy_luts(progress)?;
    let slot = match kind {
    ...
    match slot.get_or_init(move || {
        ...
        WrfTMatrixRawEvaluator::new_with_p3(tables.bundle(), p3)
            .map_err(|error| format!("validate S-pack P3 raw property evaluator: {error}"))
    }) {
        Ok(evaluator) => Ok(evaluator.clone()),
        Err(error) => Err(error.clone()),
    }
...
fn legacy_luts(
    ...
    load_or_download_legacy_property_luts(&property_tmatrix_pack_cache_dir(), progress)
}

#[cfg(test)]
fn legacy_luts(
    _progress: &(impl Fn(&str) + ?Sized),
) -> Result<&'static LegacyPropertyTMatrixLuts, String> {
    embedded_luts()
}

/// Conservative resident bytes for the five decoded on-demand tables. Source
/// file buffers and the downloaded ZIP are dropped after decode, so the old
/// embedded-source-plus-decoded 2x accounting is no longer appropriate. The
/// compatibility name is retained for memory-estimator callers and provenance.
#[must_use]
pub const fn embedded_lut_memory_bytes() -> usize {
    LEGACY_PROPERTY_TABLE_SOURCE_BYTES
}

#[cfg(test)]
static TEST_REPOSITORY_LUTS: OnceLock<Result<LegacyPropertyTMatrixLuts, String>> = OnceLock::new();

/// Offline test seam: normal focused tests read the exact repository payloads
/// without embedding them or depending on a published GitHub release asset.
#[cfg(test)]
fn embedded_luts() -> Result<&'static LegacyPropertyTMatrixLuts, String> {
    match TEST_REPOSITORY_LUTS
        .get_or_init(crate::wrf_tmatrix_legacy_pack::load_repository_legacy_property_luts)
    {
        Ok(tables) => Ok(tables),
        Err(error) => Err(error.clone()),
    }
...
mod tests {
    ...
    use radar_scattering::{AxisKind, Sha256Digest};

    use super::*;

    #[test]
    fn repository_tables_pass_whole_file_and_typed_runtime_gates() {
        let tables = embedded_luts().expect("all five repository research tables");
        ...
        tables
            .bundle()
            .validate()
            .expect("repository tables share the exact complete bundle contract");
        let resolved = PropertyTMatrixTables {
            storage: PropertyTMatrixTableStorage::LegacySDataPack(tables),
        };
        assert_eq!(
            resolved.identity(),
        ...
        resolved
            .borrowed_bundle()
            .validate()
            .expect("resolved legacy handle borrows the on-demand bundle");
        assert_eq!(
            embedded_lut_memory_bytes(),
            LEGACY_PROPERTY_TABLE_SOURCE_BYTES
        );
    }
    ...
    fn source_routing_accepts_only_declared_exact_frequencies() {
        assert_eq!(
            validate_property_tmatrix_table_source_frequency(
                PropertyTMatrixTableSourceKind::LegacyEmbeddedSResearchV1,
                S_BAND_RESEARCH_FREQUENCY_HZ,
            )
        ...
        for band in [TMatrixResearchBand::C, TMatrixResearchBand::X] {
            let error = validate_property_tmatrix_table_source_frequency(
                PropertyTMatrixTableSourceKind::LegacyEmbeddedSResearchV1,
                band.exact_frequency_hz(),
            )
    ...
    fn real_p3_fixture_builds_all_active_cells_on_cuda() {
        ...
        .expect("load exact on-demand S-band property tables");
