...
//!   errors and it exits — IDENTICAL to the hand-rolled `self.x_rx = None`
//!   contract this type replaces, so cancellation semantics are preserved
//!   exactly.
...
impl<T> WorkerTx<T> {
    ...
    pub fn send(&self, value: T) -> Result<(), Cancelled> {
        let result = self.sender.send(value).map_err(|_| Cancelled);
        self.ctx.request_repaint();
        result
    }

    /// Raw sender, for pre-existing worker bodies whose signatures must stay
    /// untouched (e.g. a worker that throttles its own progress sends). New
    /// code sends through [`WorkerTx::send`].
...
impl<T: Send + 'static> WorkerSlot<T> {
    ...
    pub fn poll(&mut self) -> SlotPoll<T> {
    ...
    pub fn inject_for_test(&mut self, rx: mpsc::Receiver<T>) {
        self.rx = Some(rx);
        self.started = Some(Instant::now());
    }
}
...
/// messages and the slot stays busy until a terminal one (fits progress
/// streams and decoded-frame batches).
pub struct StreamSlot<T: SlotMessage> {
    rx: Option<mpsc::Receiver<T>>,
    started: Option<Instant>,
    label: &'static str,
}
...
impl<T: SlotMessage + Send + 'static> StreamSlot<T> {
    ...
    pub fn drain(&mut self, budget: Duration) -> (Vec<T>, StreamState) {
        let Some(rx) = &self.rx else {
