...
Built-in presets are **BowEcho default** and **Generic branded app**. The generic preset is a neutral starter skin with placeholder identity, links, palette, share-card text, and empty asset paths that distributors can replace before shipping. User-provided assets remain path references in Brand Kit JSON; import/export preserves those paths rather than copying files unexpectedly.
