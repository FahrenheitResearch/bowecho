...
- Live ORD spot-check on 2026-06-13: the public bucket/API still listed France 20, Belgium 2, Estonia 1, Iceland 3, Poland 10, and Ireland 2 sites for the active rolling cache. `eehar` is now bridged through Estonia's KAIA open-data API; other user-named missing IDs like `frtra`, `behel`, and `isx1` still need either ORD publication or a separate national/WIS2 provider before they should appear as live built-ins.
