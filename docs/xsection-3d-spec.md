...
6. **Dual-pol:** ρhv (CC) is non-monotone with a melting-layer minimum (ρhv 0.90–0.97 band, Giangrande, Krause & Ryzhkov 2008) — use **nearest-gate in elevation when either bracketing sample is < 0.97**, linear otherwise. Z currently interpolates in native dBZ (MRMS practice); Lakshmanan 2012 and Warren & Protat 2019 disagree about dBZ versus linear Z, so a future linear-Z path must be an explicit option rather than silently changing the default. ZDR: linear in dB.
...
- **Z (reflectivity):** interpolate in native **dBZ**, matching MRMS practice and the current engine. Warren & Protat 2019 (*JTECH* 36, 1143–1156, doi:10.1175/JTECH-D-18-0183.1) favors linear Z for convective cores; Lakshmanan 2012 (*EJSSM* 7(3), 1–8) found lower interpolation RMSE in dBZ across a much larger WSR-88D triad study. Because the literature disagrees, `LinearZ` remains a possible future explicit policy, not an implicit default change. The `reflectivity_blends_in_dbz_not_linear_z` test pins this behavior.
...
- **ΦDP display smoothing:** ΦDP is angular modulo 360°. The map's **Soften** pass therefore uses a weighted circular mean, so a 359°/1° neighborhood stays near 0° instead of becoming a false 180° feature. Scalar moments retain the arithmetic kernel, and a missing center remains missing; this does not alter the linear between-tilt rule above.
...
**Method:** I re-fetched and text-extracted the actual papers (Zhang et al. 2005 JTECH; Zhang et al. 2011 BAMS; Lakshmanan et al. 2006 WAF; Askelson et al. 2000 MWR; Trapp & Doswell 2000 JTECH; Giangrande et al. 2008 JAMC; Warren & Protat 2019 JTECH; Smith et al. 2016 BAMS; Gao, Brewster & Xue 2006 AAS + their CAPS JTECH draft; Zhang et al. 2005 AMS conference companion 81781; James et al. 2000 WAF) and checked every quoted equation/number against the extracted text. I also re-derived/numerically tested the geometry in f64. Verification used locally extracted copies of the cited papers; only public bibliographic references are retained here.
